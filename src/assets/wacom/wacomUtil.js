/* eslint-disable */
// @ts-nocheck
import WacomGSS from './../../../node_modules/wacom-util/lib/esm/wgssStuSdk';
import Q from 'q';
...
function createModalWindow(width, height) {
    ...
    console.log("Init createModalWindow: ", texto);
    ...
    dataPatient = document.createElement("p");
    textPatient = document.createTextNode(texto);
    dataPatient.appendChild(textPatient);
    ...
    canvas.appendChild(dataPatient);
...
function _createModalWindow(width, height) {
    modalBackground = document.createElement("div");
    modalBackground.id = "modal-background";
    modalBackground.className = "active";
    modalBackground.style.width = window.innerWidth + "";
...
function tabletDemo(paciente) {
    texto = paciente;
    //propFunction = passToProps;
    console.log("Datos paciente: ", texto);
    var p = new WacomGSS.STU.Protocol();
    var intf;
    ...
    WacomGSS.STU.isDCAReady()
        ...
        return WacomGSS.STU.ProtocolHelper.resizeAndFlatten(canvasImage, 0, 0, 0, 0, m_capability.screenWidth, m_capability.screenHeight, m_encodingMode, 1, false, 0, true);
    })
        .then(function (message) {
        m_imgData = message;
        ...
        if (ex instanceof DCANotReady) {
            ...
            setTimeout(function () { return close(); }, 0);
        }
    });
}
function addButtons() {
...
function drawButtons() {
    ...
    console.log("dibujamos los botones");
    ctx.save();
...
function processPoint(point, in_canvas, in_ctx) {
    ...
    if ((isDown2 && distance(lastPoint, nextPoint) > 10) ||
        (isDown && !isDown2)) {
        in_ctx.beginPath();
...
export { base64image, tabletDemo, close };

...
@Component({
  selector: 'sacyl-consumption-establishment',
  standalone: true,
       imports:[
            CommonModule,
            FormsModule,
  ...
  templateUrl: './consumption-establishment.component.html',
  styleUrls: ['./consumption-establishment.component.css']
})
export class ConsumptionEstablishmentComponent implements ListingsComponentsInterface {
  ...
  filterUpdate() {
    this.loading = true;
    this.reportsService.getConsumptionEstablishment(this.filter).subscribe(data => {
      if (data.length === 0) {
        this.noResults = true;
        this.reportList = [];
      } else {
  ...
  exportToExcel() {
    ...
    exportDataToExcel(this.reportList, columns, 'consumoEstablecimiento.xlsx');

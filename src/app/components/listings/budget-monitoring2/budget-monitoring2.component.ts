...
@Component({
  selector: 'sacyl-budget-monitoring2',
  standalone: true,
         imports:[
              CommonModule,
              FormsModule,
  ...
  templateUrl: './budget-monitoring2.component.html',
  styleUrls: ['./budget-monitoring2.component.css']
})
export class BudgetMonitoring2Component implements ListingsComponentsInterface {
  ...
  filterUpdate() {
    this.loading = true;
    this.reportsService.getBudgetMonitoring(this.filter, 2).subscribe(data => {
      if (data.length === 0) {
        this.noResults = true;
        this.reportList = [];
      } else {
  ...
  exportToExcel() {
    ...
    exportDataToExcel(this.reportList, columns, 'consumoPresupuestario2.xlsx');
